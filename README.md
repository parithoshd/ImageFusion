...
summarization_prompt = (
    ...
    ignore the background. Or else, If it contains only a background, describe the scene. If it contains only an object, describe
    the object's visual details. Keep it restricted to 20 words or so."
    f"\"{body.prompt.strip()}\" "
    "Blend the above elements into the main prompt above, create one continuous image. Do not introduce extra people or unrelated changes.
    DO NOT split the image into multiple panels or sections. Do not create 'panel layout' or a 'split panel' or'collage layout'."
        )
```
